 ...
 * Encapsulates the _children keyword datamodel element defined in SCORM 1.2 and
 * SCORM 2004.
 * 
...
public class Children extends DMElement {
	 ...
	 * serialVersionUID
	 */
	private static final long serialVersionUID = 2811770400340527986L;
	 ...
	 */
	private List<String> mChildrenList = null;
	 ...
	 * 
	 * @param iChildren
	 *            Describes the list of children (dot-notation bindings) of some
	 *            data model element.
	 * 
	 */
	public Children(List<String> iChildren) {
		if (iChildren.size() > 0) {
	...
	/**
	 * Constructs a <code>_children</code> keyword data model element consisting
	 * of a SCORM conformant set of child data model element names. If the list
	 * of children is empty (size <= 0), then a Illegal Argument Exception is
	 * thrown.
	 * 
	 * @param iChildren
	 *            Describes the list of children (dot-notation bindings) of some
	 ...
	 * 
	 * @param iRandom
	 *            Describes if the list of children will provided in a random
	 *            order when a <code>GetValue()</code> operation is invoked.
	 */
	public Children(List<String> iChildren, boolean iRandom) {
		if (iChildren.size() > 0) {
			mChildrenList = iChildren;
			mRandomize = iRandom;
		} else
