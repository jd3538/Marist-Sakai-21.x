...
public class SCORM_2004_NAV_DMElement extends DMElement implements Serializable {
	 ...
	 * serialVersionUID
	 */
	private static final long serialVersionUID = -2873624865598548352L;
	...
	public int equals(RequestToken iValue, boolean iValidate) {
		...
		if (mValue != null && iValue != null) {
			...
			if (equal) {
				...
				if (equal) {
					StringBuilder compareWith = new StringBuilder(iValue.getValue());
					...
					if (mDescription.mValidator == null) {
						...
						equal = mDescription.mValidator.compare(compareWith.toString(), mValue, null);
					}
				}
			}
		...
		if (result == DMErrorCodes.NO_ERROR || result == DMErrorCodes.SPM_EXCEEDED) {
			if (mDescription.mBinding.equals("request")) {
				...
				if (mValue.equals("choice")) {
					if (mDelimiters != null) {
						DMDelimiter del = mDelimiters.get(0);
					...
					if (mValue != null) {
						...
						if (mDM.getNavRequests() != null) {
							if (mDescription.mBinding.equals("continue")) {
								if (mDM.getNavRequests().isContinueEnabled() || mDM.getNavRequests().isContinueExitEnabled()) {
									oInfo.mValue = "true";
								} else {
									oInfo.mValue = "false";
								}
								...
								if (mDM.getNavRequests().getChoice() != null) {
									boolean ok = mDM.getNavRequests().getChoice().get(target) != null;
					...
					if (idx >= mRecords.size()) {
						...
						if (idx == mRecords.size() && !(mDescription.mMaximum && mRecords.size() == mDescription.mSPM)) {
							...
							if (ok) {
								...
								if (isWriteable || ioRequest.isAdminRequest()) { // Create the new record
									DMElementDescriptor desc = (DMElementDescriptor) mDescription.clone();
									desc.mSPM = -1;

									SCORM_2004_NAV_DMElement element = new SCORM_2004_NAV_DMElement(desc, this, mDM);
		...
		if (result == DMErrorCodes.NO_ERROR || result == DMErrorCodes.SPM_EXCEEDED) {
			...
			if (mDescription.mBinding.equals("request")) {
				...
				if (mValue.equals("choice")) {
					...
					DMDelimiter target = mDelimiters.get(0);
		...
		if (mDescription.mDelimiters != null) {
			...
			for (; i < iValue.getDelimiterCount() && found; i++) {
				...
				for (int j = 0; j < mDescription.mDelimiters.size(); j++) {
					DMDelimiterDescriptor desc = mDescription.mDelimiters.get(j);
