...
public class LangStringValidator extends DMTypeValidator implements Serializable {
	...
	/**
	    ...
	    */
	private static final String ISO_COUNTRIES = ",AD.020AND,AE.784ARE,AF.004AFG,AG.028ATG,AI.660AIA,AM.051ARM,AN.530ANT"
	 ...
	 */
	private static final String ISO_LANGUAGES = ",aaaar,ababk,aeave,afafr,akaka,amamh,anarg,arara,asasm,avava,ayaym,azaze"
	 ...
	 */
	private static final String ISO_EXTENDED_LANGUAGES = "ace,ach,ada,ady,afa,afh,akk,alb,ale,alg,ang,apa,arc,arm,arn,arp,art,arw,"
	 ...
	 */
	private static final String IANA_LANGUAGES = "i,x,art-lojban,az-arab,az-cyrl,az-latn,cel-gaulish,de-1901,de-1996,"
					...
					if (!lang1.equals("x")) {
						...
						if (!done) {
							...
							if (mod1 == 3 && mod2 == 3) {
								...
								if (lang1.charAt(0) == 'q') {
									if (lang1.charAt(1) >= 'a' && lang1.charAt(1) <= 't') {
										if (lang1.charAt(2) >= 'a' && lang1.charAt(2) <= 'z') {
						...
						if (!done) {
							int idx1 = ISO_LANGUAGES.indexOf(lang1) - mod1;
							int idx2 = ISO_LANGUAGES.indexOf(lang2) - mod2;

							if (idx1 < 0 || idx2 < 0 || idx1 != idx2) {
								// Look at extended languages
								idx1 = ISO_EXTENDED_LANGUAGES.indexOf(lang1) - mod1;
								idx2 = ISO_EXTENDED_LANGUAGES.indexOf(lang2) - mod2;
			...
			if (!done) {
				...
				if (StringUtils.isNotBlank(country1) && StringUtils.isNotBlank(country2)) {
					...
					if (!done) {
						...
						int idx1 = ISO_COUNTRIES.indexOf(country1) - mod1;
						int idx2 = ISO_COUNTRIES.indexOf(country2) - mod2;
	...
	public int validate(String iValue) {
		...
		if (iValue != null) {
			...
			if (!done) {
				...
				if (iana.length() > 0) {
					iana += ",";
					idx = IANA_LANGUAGES.indexOf(iana);
				}
				...
				if (toTest) {
					...
					int found = ISO_LANGUAGES.indexOf(lang);

					if (found == -1) {
						// Check the extended strings
						found = ISO_EXTENDED_LANGUAGES.indexOf(lang);
			...
			if (!done) {
				...
				if (StringUtils.isNotBlank(country)) {
					...
					if (!done) {
						...
						int found = ISO_COUNTRIES.indexOf(country);
