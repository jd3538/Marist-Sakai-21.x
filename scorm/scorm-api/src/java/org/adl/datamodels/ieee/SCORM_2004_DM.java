...
public class SCORM_2004_DM extends DataModel implements Serializable {
	...
	public void init(IValidatorFactory validatorFactory) {
		...
		// location
		desc = new DMElementDescriptor("location", null, SHORT_SPM, new SPMRangeValidator(SHORT_SPM));
		children.add(desc);
		...
		// completion_status
		String[] vocab = { "unknown", "completed", "not attempted", "incomplete" };
		...
		// interaction type
		String[] typeFormat = { "true-false", "choice", "fill-in", "long-fill-in", "matching", "performance", "sequencing", "likert", "numeric", "other" };
		...
		// launch_data
		desc = new DMElementDescriptor("launch_data", null, LONG_SPM, new SPMRangeValidator(LONG_SPM));
		desc.mIsWriteable = false;
		...
		// learner_id
		desc = new DMElementDescriptor("learner_id", null, LONG_SPM, new URIValidator(LONG_SPM, "long_identifier_type"));
		desc.mIsWriteable = false;
		...
		desc = new DMElementDescriptor("language", "", SHORT_SPM, new LangStringValidator(true));
		...
		// suspend_data
		desc = new DMElementDescriptor("suspend_data", null, SUSPEND_DATA_SPM, new SPMRangeValidator(SUSPEND_DATA_SPM));

		element = new SCORM_2004_DMElement(desc, null, this);
		mElements.put(desc.mBinding, element);

		// time_limit_action
		String[] actionFormat = { "exit,message", "continue,message", "exit,no message", "continue,no message" };
	...
	private boolean confirmUniqueness(DMRequest ioRequest, RequestToken iValue) {
		...
		newRequest.append("._count");

		DMRequest req = null;
		try {
			req = new DMRequest(newRequest.toString());
		...
		for (int i = 0; i < iValue.getDelimiterCount(); i++) {
			...
			value.append(del.showDotNotation());
		}
		...
		for (int i = 0; i < (count - 1); i++) {
			...
			try {
				req = new DMRequest(newRequest.toString(), value.toString());
...
//	}
			...
			if (!iRequest.hasMoreTokens()) {
				...
				if (tok.getType() == RequestToken.TOKEN_VALUE) {
					...
					if (pi.mElement.getDescription() != null) {
						...
						if (result == DMErrorCodes.NO_ERROR && pi.mElement.getDescription().mWriteOnce) {
							if (pi.mElement.isInitialized()) {
								// Need to check if the current value is different
								// than the provided value
								if (!(((SCORM_2004_DMElement) pi.mElement).getInternalValue()).equals(tok.getValue())) {
	...
	public String toString() {
		...
		for (Entry<String, DMElement> entry : entrySet) {
			...
			DMElement value = entry.getValue();
			b.append(value.toString());
			List<DMElement> allChilden = value.getAllChilden();
			for (DMElement dmElement : allChilden) {
				b.append(",\n");
				b.append(dmElement.toString());
			}
			first = false;
		}
		b.append("]");
