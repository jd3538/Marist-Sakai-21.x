...
public class RequestToken {
	 ...
	 * type tokens.</b>
	 */
	private List<RequestDelimiter> mDelimiters = null;
	...
	public int getDelimiterIndex(String iName) {
		...
		if (mDelimiters != null) {
			...
			for (int i = 0; i < mDelimiters.size(); i++) {
				RequestDelimiter del = mDelimiters.get(i);
	 ...
	 */
	public List<RequestDelimiter> getDelimiters() {
		return mDelimiters;
	}
	...
	public String getDelimiterValueAt(int iIndex) {
		...
		if (mDelimiters != null) {
			if (iIndex < mDelimiters.size()) {
				RequestDelimiter del = mDelimiters.get(iIndex);
	...
	public String toString() {
		return "RequestToken [mValue=" + mValue + "]";
	}
