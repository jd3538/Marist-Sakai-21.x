			...
			if (ioThisActivity.getChildren(false) != null) {
				...
				if (mRollupRules != null) {
					...
					for (int i = 0; i < mRollupRules.size(); i++) {
						...
						if (rule.mAction == SeqRollupRule.ROLLUP_ACTION_SATISFIED || rule.mAction == SeqRollupRule.ROLLUP_ACTION_NOTSATISFIED) {
							satisfiedRule = true;
						}

						if (rule.mAction == SeqRollupRule.ROLLUP_ACTION_COMPLETED || rule.mAction == SeqRollupRule.ROLLUP_ACTION_INCOMPLETE) {
							completedRule = true;
						}
					}
				}
