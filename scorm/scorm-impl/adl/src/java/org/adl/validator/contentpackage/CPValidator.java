 ...
 * <strong>Description: </strong>The <code>CPValidator</code> determines whether
 * the content package test subject is conformant with the Content Package
 * Application Profiles( resource, content aggregation) as defined in the
 * Content Aggregation Model of the SCORM. The Content Package Validator
...
public class CPValidator extends ADLSCORMValidator implements IValidator {
	...
	// Logger.getLogger("org.adl.util.debug.validator");

	private static Log mLogger = LogFactory.getLog(CPValidator.class);

	/**
	 * This attribute serves as the object that contains the information
	 * required for the SCORM Validation checks, containing the DOM of the XML
	 * rules that are neccessary for meeting conformance to each of the
	 ...
	 * identifiers.
	 */
	private List<String> mOrganizationIdentifierList;
	 ...
	 */
	private List<Node> mResourceNodes;
	 ...
	 */
	private List<String> mRequiredFilesList;
	...
	/**
	 * This attribute holds the identifier values of the resource elements. It
	 * is used to validate that the dependency element references a resource
	 ...
	 */
	private List<String> mResourceIdentifierList;

	/**
	 * This List holds all of the Resource Identifiers listed in the manifest.
	 * It will be used to verify that all the resources have been referenced for
	 * the dangling resource test (excess baggage).
	 */
	private List<String> mManifestResourceIdentifierList;
	 ...
	 */
	private Hashtable<String, List<String>> mResourceTable;

	/**
	 * Indicates if the xsi:schemaLocation was declared in the XML instance. The
	 * xsi:schemalocation attribute provides a hint to the XML Parser as to
	 * where the controlling documents are located. The xsi:schemalocation
	 ...
	 * 
	 * @param iEnvironmentVariable
	 *            the value specific to the location of the using system's
	 *            install.
	 */
	 ...
	 * 
	 * @param iHrefValue
	 *            href value to apply xml:base values to
	 * @return String that contains the href value pre-appended with the
	 *         xml:base
	 */
	...
	private void checkBucketAttributes(Node iBucketNode) {
		...
		for (int i = 0; i < numAttr; i++) {
			...
			if (currentAttrName.equals("bucketID") || currentAttrName.equals("bucketType")) {
				...
				if (!foundValidChar) {
					msgText = "Attribute \"" + currentAttrName + "\" must " + "contain valid characters, all whitespace found.";
	 ...
	 *            if the application type is content aggregation then we have
	...
	private void checkForExcessBaggage(String iApplicationProfileType) {
		...
		if (mFileList.size() > 0) {
			...
			if (mFileList.size() == 1) {
				msgText = "All files included in the content package should " + "be declared and referenced in the manifest when "
				        + "interchanging packages.  The following file is " + "contained in the content package but was not "
				        + "referenced in the manifest.  It appears that this " + "file may be extraneous.  Appropriate action may "
				        + "need to be taken to correct this problem";
			}
			// If there is more than one file in the list
				...
				msgText = "All files included in the content package should " + "be declared and referenced in the manifest when "
				        ...
				        + "referenced in the manifest.  It appears that these " + "files may be extraneous.  Appropriate action may "
				        + "need to be taken to correct this problem:";
			}
			...
			// we've displayed the warning, now list the files creating the
			// warning
			for (int i = 0; i < mFileList.size(); i++) {
				mLogger.debug(mFileList.get(i));
				DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.OTHER, mFileList.get(i)));
			}
		...
		if (mManifestResourceIdentifierList.size() > 0 && iApplicationProfileType.equals("contentaggregation")) {
			if (mManifestResourceIdentifierList.size() == 1) {
				...
				msgText = "All files included in the content package should " + "be declared and referenced in the manifest when "
				        + "interchanging packages.  The following resource is " + "listed in the manifest but was not referenced by "
				        + "an \"item\" or \"dependency\".  It appears that this " + "may be a dangling reference.  Appropriate action "
				        + "may need to be taken to correct this problem";
			}
			// If there is more than one dangling resource in the list
				...
				msgText = "All files included in the content package should " + "be declared and referenced in the manifest when "
				        ...
				        + "referenced by any \"item\" or \"dependency\".  It " + "appears that these may be dangling references.  "
			...
			for (int j = 0; j < mManifestResourceIdentifierList.size(); j++) {
				mLogger.debug(mManifestResourceIdentifierList.get(j));
				DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.OTHER, mManifestResourceIdentifierList.get(j)));
			}
	...
	/**
	 * This method assists with the application profile check for validation
	 * that a sco resource is referenced by an item identifierref.
	 * 
	 * @param idrefValue
	 *            - the idrentifierref value to be matched to a sco resource.
	...
	private boolean checkForReferenceToSco(String idrefValue) {
		...
		for (int i = 0; i < len; i++) {
			Node currentResource = mResourceNodes.get(i);
	 ...
	 * 
	 * @param iDirectory
	 *            The directory to check to see if the required files are
	 *            present.
	 * @return boolean - true if the required files are present, false otherwise
	 */
	private boolean checkForRequiredFiles(String iDirectory, List<String> iDeclaredNamespaces) {
		...
		DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.INFO, Messages.getString("CPValidator.63")));
		...
		List<String> locationMD = mdHandler.getLocationMD(super.getDocument().getDocumentElement());
		...
		if (!locationMD.isEmpty()) {
			...
			for (int i = 0; i < locationMDSize; i++) {
				String currentMetadataData = locationMD.get(i);
				...
				if (!adldomparser.getIsWellformed()) {
					...
					DetailedLogMessageCollection.getInstance().addMessage(
					        new LogMessage(MessageType.FAILED, Messages.getString("CPValidator.64", currentMetadataData, currentMetadataData)));
				}
			}
	 ...
	 * for the required trailing slash '/'
	 * 
	 * @param iName
	 ...
	 * 
	 * @param iURIString
	 *            The URI value of the href attribute
	 ...
	 * @param iURIString
	 *            the URI passed in
	 * @return boolean - result of the local URL check. True if the local checks
	 *         passed, false otherwise.
	 */
	private boolean checkLocalURL(String iURIString) {
		...
		if (iURIString.charAt(0) == '/') {
			...
			// after the parameters have been stripped off we want to check to
			// see if
			// this URI is different from any others by case (upper/lower)

			// if the absolutePath is in the list then we fall through, if it
			// isn't then
			...
			if (!mHrefCaseSensitiveList.contains(absolutePath)) {
				...
				if (mHrefCaseSensitiveList.containsIgnoreCase(absolutePath)) {
					...
					DetailedLogMessageCollection.getInstance().addMessage(
					        new LogMessage(MessageType.WARNING, Messages.getString("CPValidator.528", iURIString)));
				} else {
					// it's not in the list at all, so add it for future
					// reference
	...
	private boolean checkResourceAttributes(Node iResourceNode, NamedNodeMap iAttrList) {
		...
		if (mRulesValidator.getApplicationProfile().equals("contentaggregation")) {
			// special checks to be enforced when an <item> references a
			// <resource>

			// retrieve resource.identifier value and compare to idref values in
			// the valid mValidIdrefs List. If a match is found, it is assumed
			...
			if (!resourceID.isEmpty()) {
				...
				for (int i = 0; i < mValidIdrefs.size(); i++) {
					String currentIdref = mValidIdrefs.get(i);
	...
	private boolean checkResourceFileHref(Node iResourceNode) {
		...
		if (!resourceHref.isEmpty()) {
			...
			if (!StringUtils.startsWith(resourceHref, "http:")
			        && !StringUtils.startsWith(resourceHref, "https:")
	 ...
	 * 
	 * @param iElementName
	 *            Name of the element being checked for spm
	 * @param iElementValue
	 *            value being checked for smp
	 * @param iSPMRule
	 *            value allowed for spm ( value retrieved from rules )
	 * @param iAmAnAttribute
	 *            flags determines if its an attribute (true), or an element
	 *            that is being validated for valid vocabulary tokens.
	 ...
	 * 
	 * @param iManifestFileName
	 *            - The URI location of the manifest that is to be parsed for
	 *            wellformedness and validation to the schema(s).
	 ...
	 * 
	 * @param iName
	 *            Name of the element/attribute being checked for valid
	 *            vocabulary.
	 * @param iValue
	 *            Vocabulary string value that exists for the element/attribute
	 *            in the test subject
	 * @param iVocabValues
	 *            List containing a list of the valid vocabulary values for
		...
		for (int i = 0; i < iVocabValues.size(); i++) {
			if (iValue.equals(iVocabValues.get(i))) {
				...
				if (iName.equals("schemaversion")) {
					if (iVocabValues.get(i).equals("CAM 1.3")) {
	...
	private boolean compareToRules(Node iTestSubjectNode, String iPath) {
		...
		case Node.DOCUMENT_NODE: {
			...
			DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.INFO, Messages.getString("CPValidator.131", rootNodeName)));

			mLogger.debug("PASSED: Multiplicity for element <" + rootNodeName + "> has been verified");
			DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.PASSED, Messages.getString("CPValidator.135", rootNodeName)));
		...
		case Node.ELEMENT_NODE: {
			...
			for (int i = 0; i < numAttr; i++) {
				...
				mLogger.debug("Processing the [" + currentAttrNode.getNamespaceURI() + "] " + currentNodeName + " attribute of the <" + parentNodeName
				        + "> element.");
			...
			if (parentNodeName.equalsIgnoreCase("manifest")) {
			...
			} else if (parentNodeName.equalsIgnoreCase("organizations") && (mRulesValidator.getApplicationProfile()).equals("contentaggregation")) {
				...
				if (multiplicityUsed < 1) {
					...
					DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.FAILED, Messages.getString("CPValidator.198", "default")));

					result = false;
				}
			} else if (parentNodeName.equalsIgnoreCase("organization") && (mRulesValidator.getApplicationProfile()).equals("contentaggregation")) {
				multiplicityUsed = getMultiplicityUsed(attrList, "identifier");
				if (multiplicityUsed < 1) {
					mLogger.debug("FAILED: Mandatory attribute \"identifier\" " + "could not be found");
					DetailedLogMessageCollection.getInstance().addMessage(
			...
			for (int j = 0; j < numAttr; j++) {
				...
				if (dataType.equalsIgnoreCase("idref") || dataType.equalsIgnoreCase("id") || dataType.equalsIgnoreCase("vocabulary")
					...
					if (dataType.equalsIgnoreCase("idref")) {
						...
						if (currentNodeName.equalsIgnoreCase("identifierref") && parentNodeName.equalsIgnoreCase("dependency")) {
						...
						List<String> vocabAttribValues = mRulesValidator.getAttribVocabRuleValues(parentNodeName, iPath, currentNodeName);
			...
			if (children != null) {
				...
				else if (parentNodeName.equalsIgnoreCase("organizations") && (mRulesValidator.getApplicationProfile()).equals("resource")) {
					multiplicityUsed = getMultiplicityUsed(iTestSubjectNode, "organization");
					...
					if (currentChildName != null) {
						...
						if ((currentChildName.equals("bucket")) && (parentNodeName.equals("resource"))) {
							...
							if (!typeS.equalsIgnoreCase("sco")) {
								...
								msgText = "The <" + currentChildName + "> shall" + " only exist in a resource that is " + " scormType = \"sco\".";
						...
						if (dataType.equalsIgnoreCase("parent") || dataType.equalsIgnoreCase("vocabulary") || dataType.equalsIgnoreCase("text")
						        || dataType.equalsIgnoreCase("sequencing") || dataType.equalsIgnoreCase("metadata") || dataType.equalsIgnoreCase("decimal")) {
	 ...
	 * 
	 * @param rootDirectory
	 *            A string containing the path of the root directory of the
	...
	private void createManifestFileList(String rootDirectory) {
		...
		if (fileOrFolder.isDirectory() && !fileOrFolder.getName().equals("common") && !fileOrFolder.getName().equals("vocab")
		        && !fileOrFolder.getName().equals("extend") && !fileOrFolder.getName().equals("unique")) {
		...
		// if it is a file and it isn't an xsd or the manifest add it to the
		// list
		if (fileOrFolder.isFile() && !fileOrFolder.getName().endsWith(".xsd") && !fileOrFolder.getName().equals("imsmanifest.xml")
		        && !fileOrFolder.getName().endsWith(".dtd")) {
	 ...
	 * existance at the root of the package.
	 * 
	 * @param iSchemaLocations
	 *            -- the string value that houses the schemaLocations
	 * @param iDirectory
	 *            -- the base directory of the test suite that is used to build
	 *            the location of the schemas.
	 * @return integer - 1 if files are found, 0 if files are not found.
	 */
	private int findAndLocateRequiredFiles(String iSchemaLocations, String iDirectory, List<String> iDeclaredNamespaces) {
		int result = 1;
		File currentFile;
		...
		if (!mSchemaLocExists) {
			...
			// retrieve namespace section of schemaLocation value to determine
			// which
			...
			for (int x = 0; x < namespace.length; x++) {
				if (namespace[x].startsWith("http")) {
					// we have a namespace that needs compared to the declared
					// namespaces
					if (iDeclaredNamespaces.contains(namespace[x])) {
						// remove namespace and schemalocation from
	 ...
	 * @param resourceNode
	 *            &lt;resource&gt; element
	 * @param resourcesXMLBase
	 *            xml:base attribute value of the &lt;resources&gt; element
	 * @param fileHrefList
	 *            list of files being tracked for the &lt;resource&gt; element
	 * @param resourceNodes
	 *            List containing all the &lt;resource&gt; elements that
	 *            belong to the &lt;resources&gt; element
	 * 
	 * @param resourcesVisited
	 *            list contains the &lt;resource&gt; elements tracked for
	 *            dependency referencing to prevent circular dependencies.
	 */
	private void findFileHrefs(Node resourceNode, String resourcesXMLBase, List<String> fileHrefList, List<Node> resourceNodes, List<String> resourcesVisited) {
		// we must manually retrieve the resource xml:base attribute value
		// the applyXMLBase is not aware of it at this time of processing
		...
		// add all file elements that are direct children of resource to the
		// array
		List<Node> fileNodes = DOMTreeUtility.getNodes(resourceNode, "file");

		if (!fileNodes.isEmpty()) {
			Node currentFileChild;
			int len = fileNodes.size();
			for (int k = 0; k < len; k++) {
				currentFileChild = fileNodes.get(k);
		...
		// elements
		List<Node> dependencyNodes = DOMTreeUtility.getNodes(resourceNode, "dependency");
		if (!dependencyNodes.isEmpty()) {
			// determine identifierref and add to the hashtable based on this
			// key
			Node currentDependency;
			int len = dependencyNodes.size();
			mLogger.debug("number of dependency is " + len);
			for (int y = 0; y < len; y++) {
				currentDependency = dependencyNodes.get(y);
				...
				if (!dependencyIDRef.equals(resourceID)) {
					...
					for (int k = 0; k < numChildren; k++) {
						current = resourceNodes.get(k);

						currentResourceID = DOMTreeUtility.getAttributeValue(current, "identifier");

						mLogger.debug("Comparing resource id " + currentResourceID + " to this dependencyIDRef " + dependencyIDRef);

						if (currentResourceID.equals(dependencyIDRef)) {
							// first determine if we have a circular dependency
							// before settting the match
					...
					// retrieve all the file elements of the resource referenced
					// by
					// the dependency and add them to the list if we have a
					// match
					if (dependencyMatch != null) {
						findFileHrefs(dependencyMatch, resourcesXMLBase, fileHrefList, resourceNodes, resourcesVisited);
					}
				}
			}
		}
	 ...
	 * the user chooses to validate SCOs.
	 * 
	 ...
	 * information
	 * 
	 * @param iRolledUpDocument
	 *            - Dom with manifest rollup
	 * @param iDefaultOrganizationOnly
	 *            - boolean describing which organization to read from
	 * @param iRemoveAssets
	...
	/**
	 * This method extracts the selected test subject, in the form of a zip
	 * file, to a temporary "/PackageImport" directory in order to perform
	 * validation.
	 * 
	 * @param iPIF
	 *            The content package zip test file URI
	 * @param encoding
	 * @return extractDir returnst the path of the directory the file is
	 *         extracted to
	 */
	private String importContentPackage(String iPIF, String encoding) {
		// get the extract dir
		...
		UnZipHandler uzh = new UnZipHandler(iPIF, extractDir, encoding);
		uzh.extract();
	 ...
	 * &lt;dependency&gt; elements) that belong to each resourceID. If a
	 * resource does not contain an association of files, then an key will not
	 * be added to the hashtable for that resource. This method assists in the
	 * checking of the requirement that if a resource is local to a content
	 * package, than it must contain an identifical &lt;file&gt; element in
	 * association with it.
	 * 
	...
	private void populateResourceTable(Node iResourcesNode) {
		...
		List<Node> resourceNodes = DOMTreeUtility.getNodes(iResourcesNode, "resource");
		List<String> fileHrefList;
	 ...
	 * 
	 * @param iResourcesNode
	 *            The &lt;resources&gt; node that parents the &lt;resource&gt;
	 *            elements. <br>
	 */
	private void trackResourceIdentifiers(Node iResourcesNode) {
		...
		if (iResourcesNode != null) {
			...
			for (int i = 0; i < resourceNodesSize; i++) {
				Node currentChild = resourceNodes.get(i);
	...
	/**
	 * Recursively walk the (sub)manifest node and track resource.href,
	 * file.href and adlcp:location URI values. We will attempt to detect these
	 * files. If found, they will be removed from the mFileList in order to
	 * account for excessive baggage that is caused from (sub)manifests.
	 * 
	 * @param iSubmanifestNode
	 *            the (sub)manifest element node
	 * @param iSubmanifestURIList
	 *            used to keep track of file and href references in
	 *            (sub)manifests to exclude them from the excess
	 *            baggage/dangling resource checks
	 */
	private void trackSubManifest(Node iSubmanifestNode, List<String> iSubmanifestURIList) {
		...
		if (iSubmanifestNode.getNodeName().equals("manifest")) {
			...
			if (resourcesNode != null) {
				...
				List<Node> resourceNodes = DOMTreeUtility.getNodes(resourcesNode, "resource");

				// Loop through the resource elements to retrieve the
				// href attribute values

				for (int i = 0; i < resourceNodes.size(); i++) {
					Node currentResourceChild = resourceNodes.get(i);
					...
					mLogger.debug("Just added " + xmlBase + resourceHref + "to the sub List");

					// get children to add file href if it exists
					List<Node> fileNodes = DOMTreeUtility.getNodes(currentResourceChild, "file");

					// Loop through the file elements to retrieve the
					// href attribute values

					for (int j = 0; j < fileNodes.size(); j++) {
						Node currentFileChild = fileNodes.get(j);
						String fileHref = DOMTreeUtility.getAttributeValue(currentFileChild, "href");

						iSubmanifestURIList.add(xmlBase + fileHref);

						mLogger.debug("Just added " + xmlBase + fileHref + "to the sub List");
					}
				}
			}
		}
		...
		for (int m = 0; m < iSubmanifestURIList.size(); m++) {
			mFileList.remove(iSubmanifestURIList.get(m));
		}
	}
	 ...
	 * 
	 * @param iFileName
	 *            The name of the SCORM Content Package test subject
	 * @param iTestType
	 *            The type of test subject ( pif or media )
	 * @param iApplicationProfileType
	 *            The Application Profile type of the test subject (content
	 *            aggregation or resource )
	 * @param iManifestOnly
	 *            The boolean describing whether or not the IMS Manifest is to
	 *            be the only subject validated. True implies that validation
	 *            occurs only on the IMS Manifest (checks include
	...
	public boolean validate(String iFileName, String iTestType, String iApplicationProfileType, boolean iManifestOnly, String encoding) {
		...
		if (iTestType.equals("pif")) {
			rootDirectory = importContentPackage(iFileName, encoding);
		...
		try {
			...
			// Test to see whether or not the imsmanifest.xml file exists as
			// defined
			// by the pathname.
			if (!manifestFile.exists()) {
				mLogger.debug("FAILED: Required file \"imsmanifest.xml\" not " + "found at the root of the package");
				DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.FAILED, Messages.getString("CPValidator.22")));
			...
			if (manifestFound) {
				...
				// This is only done if the manifest is present, no reason to do
				// so
				// otherwise
				...
				// continue to validate only if we are dealing with an IMS
				// Manifest
				if (validRoot) {
					if (wellformed && !iManifestOnly && mPerformFullValidation) {
						// If the imsmanifest.xml is well-formed, then check to
						// make
						// sure all of the required files are present.

						// first determine if the default schemas are being
						// used. If so, warn to inform
						if (!mSchemaLocExists) {
						...
						List<String> declaredNamespaceList = super.getDeclaredNamespaces();
					...
					if (validateResult && mPerformFullValidation) {
						...
						// The imsmanifest.xml file is well-formed and all of
						// the
						// required
						// files were present, now check to see if the
						// imsmanifest.xml
						...
						if (populateResult) {
							List<String> idrefs = mManifestInfo.checkAllIdReferences();
							...
							if (!idrefs.isEmpty()) {
								...
								DetailedLogMessageCollection.getInstance().addMessage(new LogMessage(MessageType.INFO, Messages.getString("CPValidator.34")));
							}
						}
						...
						if (doXMLRulesExist) {
							...
							// Set the ADLSCORMValidator protected attribute
							// with this
							// value. This value is needed for summary logging
							// and
							// ADLValidatorOutcome
							...
							// XML rules cannot be found to perform app profile
							// checking,
							// system error
