...
public class RulesValidator implements Serializable {
	...
	public String getTaggedData(Node iNode) {
		...
		if (kids != null) {
			for (int i = 0; i < kids.getLength(); i++) {
				...
				if ((kids.item(i).getNodeType() == Node.TEXT_NODE) || (kids.item(i).getNodeType() == Node.CDATA_SECTION_NODE)) {
	 ...
	 */
	private Node retrieveAttribRuleElement(String iElementName, String iPath, String iRuleName, String iAttribName) {
		...
		for (int i = 0; i < numKids; i++) {
			...
			if (nodeName.equals(iElementName) && path.equals(iPath)) {
				...
				for (int j = 0; j < numCurrentNodeKids; j++) {
					...
					if (iRuleName.equals("min") || iRuleName.equals("max") || iRuleName.equals("spm") || iRuleName.equals("datatype")) {
						if (ruleNodeName.equals(iRuleName)) {
							resultNode = ruleNode;
							break;
						}
