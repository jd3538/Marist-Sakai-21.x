...
public class SSP_Servlet extends HttpServlet implements BucketCollectionManagerInterface, BucketManagerInterface {
	...
	public StatusInfo allocate(BucketAllocation iDescription) {
		...
		if (iDescription.getBucketID() == null || iDescription.getRequested() == null || iDescription.getOffset() != null || iDescription.getSize() != null
		        || iDescription.getValue() != null) {
			// We have a bad call
			okToProceed = false;
		...
		if (iDescription.getAllocationStatus() == SuccessStatus.FAILURE && okToUpdateDB) {
			iDescription.setReallocateFailure(true);
			track(iDescription);
		}
		// Return result
	...
	public StatusInfo appendData(String ibucketID, byte[] iData, BucketAllocation iBucketAllocation) {
		...
		// Check that only the valid parameters have been set
		if (iBucketAllocation.getBucketID() != null || iBucketAllocation.getRequested() == null || iBucketAllocation.getOffset() == null
	...
	private BucketAllocation checkForBucket(BucketAllocation iBucketAllocation) {
		...
		try {
			if (iBucketAllocation.getPersistence() == Persistence.LEARNER) {
				...
				baf = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), null, null, -1, Persistence.LEARNER, true);
				...
				baf = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), iBucketAllocation.getCourseID(), null, -1,
				        Persistence.COURSE, true);

				// Check for good course bucket
				ba = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), iBucketAllocation.getCourseID(), null, -1,
				        Persistence.COURSE, false);
			}

			else {
				// Check for locked session bucket
				baf = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), iBucketAllocation.getCourseID(),
				        iBucketAllocation.getSCOID(), -1, Persistence.SESSION, true);

				// Check for good session bucket
				ba = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), iBucketAllocation.getCourseID(),
				        iBucketAllocation.getSCOID(), -1, Persistence.SESSION, false);
			}
	...
	private StatusInfo createBucket(BucketAllocation iDescription) {
		...
		if (requested <= BucketAllocation.MAXIMUM_SIZE) {
			...
			bucket = new Bucket(iDescription.getBucketID(), iDescription.getBucketType(), minimum, iDescription.getPersistence(), reducible, requested,
			        totalSpace);
		...
		// implementation
		else if (reducible && (minimum <= BucketAllocation.MAXIMUM_SIZE)) {
			totalSpace = minimum;
	...
	public StatusInfo getData(String iBucketID, byte[] oData, BucketAllocation iBucketAllocation) {
		...
		// Check that only the valid parameters have been set
		if (iBucketAllocation.getBucketID() == null || iBucketAllocation.getRequested() != null || iBucketAllocation.getOffset() != null
		        || iBucketAllocation.getSize() != null || iBucketAllocation.getValue() == null) {
			Bucket bucket = retrieveBucket(iBucketAllocation);
	...
	private boolean persistBucket(Bucket iBucket, BucketAllocation iBucketAllocation) {
		...
		if (iBucket.getPersistence() == Persistence.COURSE) {
			...
			bucketFile += iBucketAllocation.getCourseID() + iBucketAllocation.getSCOID() + iBucketAllocation.getAttemptID();
		}
	 ...
	 * @return Bucket
	 */
	private Bucket readBucket(String iBucketID, String iStudentID, String iCourseID, String iAttemptID, String iSCOID) {
		Bucket bucket = null;
		String bucketFile = File.separator + SRTEFILESDIR;

		if ((iBucketID != null) && (iStudentID != null)) {
	...
	private Bucket retrieveBucket(BucketAllocation iBucketAllocation) {
		...
		try {
			if (iBucketAllocation.getManagedBucketIndex() >= 0) {
				ba = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), null, null, iBucketAllocation.getManagedBucketIndex(),
				        -1, false);
				...
				if (ba.getBucketID() != null) {
					if (ba.getAllocationStatus() != SuccessStatus.FAILURE) {
						...
						case Persistence.SESSION: {
							result = readBucket(iBucketAllocation.getBucketID(), iBucketAllocation.getStudentID(), iBucketAllocation.getCourseID(),
							        iBucketAllocation.getAttemptID(), iBucketAllocation.getSCOID());
							break;
						}
						case Persistence.COURSE: {
							result = readBucket(iBucketAllocation.getBucketID(), iBucketAllocation.getStudentID(), iBucketAllocation.getCourseID(), null, null);
							break;
						}
			...
			else // not a managed bucket
			{
				...
				if (okToProceed) {
					ba = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), iBucketAllocation.getCourseID(),
					        iBucketAllocation.getSCOID(), -1, Persistence.COURSE, true);
					if (ba.getBucketID() != null) {
						// entry found - the bucket is locked
						okToProceed = false;
						iBucketAllocation.setAllocationStatus(SuccessStatus.FAILURE);
				...
				// Check for locked learner bucket
				if (okToProceed) {
					ba = retrieveDBRecord(iBucketAllocation.getStudentID(), iBucketAllocation.getBucketID(), null, iBucketAllocation.getSCOID(), -1,
	 ...
	 * @return - Status or result information about the outcome of this call.
	 */
	private BucketAllocation retrieveDBRecord(String iLearnerID, String iBucketID, String iCourseID, String iSCOID, int iManagedBucketIndex, int iPersistence,
	...
	public boolean updateDBRecord(int iActivityID, int iManagedBucketIndex) {
		...
		try {
			...
			String sqlUpdateBucketIndex = "UPDATE SSP_BucketAllocateTbl SET " + "ManagedBucketIndex = ?" + " WHERE ActivityID = ?";

			stmtUpdateSSP_BucketAllocateTbl = conn.prepareStatement(sqlUpdateBucketIndex);
